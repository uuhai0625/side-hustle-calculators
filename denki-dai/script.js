...
// 数字の桁数はページごとに変わるため、幅に収まるようフォントサイズを自動調整する。
...
function drawResultImage(amountDisplay) {
  ...
  ctx.fillText('在宅ワークのパソコン電気代の目安(月額)', W / 2, 150);
  ...
  ctx.fillText('在宅ワークのパソコン電気代計算機', W / 2, 400);
  ctx.fillText('uuhai0625.github.io/side-hustle-calculators/denki-dai/', W / 2, 428);
...
btnSaveImage.addEventListener('click', async () => {
  ...
  const canvas = drawResultImage('¥' + lastMonthly.toLocaleString('ja-JP'));
  canvas.toBlob((blob) => {
    if (!blob) return;
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `denki-dai-${lastMonthly}.png`;
