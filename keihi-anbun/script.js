...
function drawResultImage(amountDisplay) {
  ...
  ctx.fillText('副業の経費按分額の目安(月額)', W / 2, 150);
  ...
  ctx.fillText('副業の経費按分計算機', W / 2, 400);
  ctx.fillText('uuhai0625.github.io/side-hustle-calculators/keihi-anbun/', W / 2, 428);
...
btnSaveImage.addEventListener('click', async () => {
  ...
  canvas.toBlob((blob) => {
    ...
    a.href = url;
    a.download = `keihi-anbun-${lastMonthly}.png`;
